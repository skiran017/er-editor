...
export const PanelHeader = ({ titleKey }: PanelHeaderProps) => {
  ...
  return (
    ...
    >
      <h2 className="text-sm font-semibold text-slate-800">{t(titleKey)}</h2>
      <button
        type="button"
        onClick={clear}
        aria-label={t('close')}
        title={t('close')}
        className="grid h-6 w-6 place-items-center rounded text-slate-500 hover:bg-slate-100 hover:text-slate-700"
